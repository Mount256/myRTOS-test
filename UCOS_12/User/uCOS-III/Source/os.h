...
typedef void 					(*OS_TASK_PTR)(void *p_arg);	/* 任务函数名 */
typedef struct os_tcb			OS_TCB;							/* TCB 重命名为大写字母格式 */
typedef struct os_rdy_list		OS_RDY_LIST;					/* 就绪列表重命名为大写字母格式 */
...
OS_EXT  OS_NESTING_CTR  OSSchedLockNestingCtr;  /* 调度器锁嵌套计数器 */
...
/* (os_core.c) 链表头部插入一个 TCB */
void OS_RdyListInsertHead (OS_TCB *p_tcb);

/* (os_core.c) 链表尾部插入一个 TCB */
void OS_RdyListInsertTail (OS_TCB *p_tcb);

/* (os_core.c) 在就绪列表中插入一个 TCB */
void OS_RdyListInsert (OS_TCB *p_tcb);

/* (os_core.c) 将 TCB 从链表头部移到链表尾部 */
void OS_RdyListMoveHeadToTail (OS_RDY_LIST *p_rdy_list);

/* (os_core.c) 链表移除一个 TCB */
...
/* (os_prio.c) 在优先级表的相应位置置位 */
void OS_PrioInsert (OS_PRIO prio);

/* (os_prio.c) 清除优先级表的相应位置 */
...
/* (os_tick.c) 往时基列表插入一个任务 TCB */
void OS_TickListInsert (OS_TCB *p_tcb, OS_TICK time);

/* (os_tick.c) 往时基列表删除一个指定的任务 TCB */
void OS_TickListRemove (OS_TCB *p_tcb);

/* (os_tick.c) 更新时基计数器，扫描时基列表中的任务延时是否到期 */
